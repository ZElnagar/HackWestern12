...
export const generateDietPlan = async (
  ...
  // REALISTIC PRICING DATA FOR ONTARIO (2024/2025 Estimates)
  const ONTARIO_PRICES_PROMPT = `
    **ONTARIO 2024/2025 GROCERY PRICE REFERENCE (CAD)**
    Use these approximate benchmarks to estimate costs. Do NOT underestimate.
    - Milk (4L): $5.89 - $6.50
    - Eggs (12 large): $4.00 - $5.50
    - Chicken Breast (boneless, skinless): $14.00 - $18.00 / kg
    - Ground Beef (Lean): $10.00 - $14.00 / kg
    - Salmon Fillet: $30.00+ / kg (Premium item)
    - Bread (Loaf, Whole Wheat): $3.00 - $4.50
    - Apples/Bananas: $2.00 - $4.00 / kg
    - Bag of Spinach (Fresh): $4.00 - $6.00
    - Frozen Vegetables (750g): $3.50 - $5.00
    - Canned Beans/Chickpeas: $1.29 - $1.99 / can
    - Rice (Basmati 2kg): $8.00 - $12.00
    - Greek Yogurt (750g tub): $6.50 - $8.00
    - Cheese (Block 400g): $6.00 - $8.00
    - Butter (454g): $6.00 - $8.50
  `;
  ...
  const userPrompt = `
    ...
    **BUDGET CONSTRAINT (CRITICAL)**:
    ...
    ${ONTARIO_PRICES_PROMPT}
    
    **STRATEGY:**
    - **Calculate costs strictly.** If the user has a low budget ($75-$100), you MUST NOT recommend salmon, expensive steaks, or out-of-season berries. Use frozen veggies, canned legumes, eggs, and bulk grains.
    - **Moderate Budget ($100-$200):** Mix of fresh/frozen, chicken breast, some ground beef.
    - **High Budget ($200+):** Organic, fresh fish, premium cuts.
    - **Enforce the limit:** If your calculated shopping list exceeds $${data.weeklyBudget}, you must perform substitutions (e.g., swap fresh spinach for frozen, swap steak for lentils) until it fits.
    ...
    2. **Plan**: Using the questionnaire constraints (allergies, religiousRestrictions, medications), produce:
       ...
       - A categorized shopping list optimized for the budget.
       - **Calculate and return 'estimatedWeeklyCost'**: The approximate total cost of the shopping list in CAD using the pricing reference provided.
...
export const sendChatMessage = async (
    ...
    const systemInstruction = `
        ...
        - If they ask about prices, quote estimates based on 2024/2025 Ontario, Canada grocery prices (e.g., Milk is ~$6, Eggs ~$5).
