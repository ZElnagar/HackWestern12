...
import { User as UserIcon, Calendar, ChevronRight, Activity, LogOut, LayoutDashboard, CheckCircle, XCircle } from 'lucide-react';
...
const ProfileHub: React.FC<ProfileHubProps> = ({ user, onViewAssessment, onLogout, onBackToDashboard, isDailyDone, onStartDailyScan }) => {
        ...
        <div className="md:col-span-2">
          <div className="bg-white rounded-2xl shadow-lg p-6 h-full">
            ...
            {user.history.filter(h => h.type === 'full').length > 0 ? (
              <div className="space-y-4">
                {user.history.filter(h => h.type === 'full').slice().reverse().map((assessment) => (
                  <button
                    key={assessment.id}
                  ...
                  >
                    <div>
                      ...
                      <p className="text-sm text-slate-500">
                        {(assessment.results as any).summary.substring(0, 60)}...
                      </p>
                    </div>
